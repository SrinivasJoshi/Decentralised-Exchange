...
export default function Home() {
	...
	//balance of tokens owned by user
	const [ethBalance, setEtherBalance] = useState(zero);
	const [cdBalance, setCDBalance] = useState(zero);
	const [lpBalance, setLPBalance] = useState(zero);
	//token reserve in contract
	const [etherBalanceContract, setEtherBalanceContract] = useState(zero);
	const [reservedCD, setReservedCD] = useState(zero);

	//Variables to keep track of liquidity to be added or removed
	//amount of token that the user wants to add to the liquidity
	const [addEther, setAddEther] = useState(zero);
	const [addCDTokens, setAddCDTokens] = useState(zero);
	//amount of token sent back after exchanging LP tokens
	...
	const getProviderOrSigner = async (needSigner = false) => {
		...
		if (chainId != 80001) {
			window.alert('Change network to Matic Mumbai tesnet');
